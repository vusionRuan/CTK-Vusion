...
void ctkDICOMDatabasePrivate::insert( const ctkDICOMItem& ctkDataset, const QString& filePath, bool storeFile, bool generateThumbnail)
{
  ...
  if ( storeFile && !q->isInMemory() && !seriesInstanceUID.isEmpty() )
    {
	  QString studyInstanceUID(QCryptographicHash::hash(ctkDataset.GetElementAsString(DCM_StudyInstanceUID).toUtf8(), QCryptographicHash::Md5).toHex());
	  QString seriesInstanceUID(QCryptographicHash::hash(ctkDataset.GetElementAsString(DCM_SeriesInstanceUID).toUtf8(), QCryptographicHash::Md5).toHex());
	  QString sopInstanceUID(QCryptographicHash::hash(ctkDataset.GetElementAsString(DCM_SOPInstanceUID).toUtf8(), QCryptographicHash::Md5).toHex());
